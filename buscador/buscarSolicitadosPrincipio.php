...
<?php
    ...
    $Principio= $_GET["principio"];//se recibe lo que se escribio en el input desde ajaxBuscador.js
    $ID_Paciente=$_GET["val_21"];//se recibe el ID_Paciente desde ajaxBUscador.js
    //echo "Buscando por las letras: " . $Principio . "<br>";
    //echo "ID_Paciente: " . $ID_Paciente . "<br>";

    if(!empty($Principio)){ 
    	$Persona= mysqli_real_escape_string($conexion,$Principio);
    	$Consulta=  "SELECT * FROM medicamentos_solicitados INNER JOIN solicitantes ON  medicamentos_solicitados.aleatorioSolicitante=solicitantes.aleatorio_Solicitante WHERE principioActivo LIKE '$Persona%'  ORDER BY principioActivo ASC";
            ...
            ?>
                        <tbody class="buscar_2">
                            <tr class="buscar_1">
                                ...
                                <td class="textoCelda_4"><?php echo $medicamento["especificaciones"];?></td>             

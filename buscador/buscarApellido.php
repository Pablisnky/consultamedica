<?php
    ...
    $Apellido= $_GET["apellido"];//se recibe lo que se escribio en el input desde ajaxBuscador.js
    $Especialista=$_GET["val_22"];//se recibe el ID_Doctor desde ajaxBUscador.js
    //echo "$Nombre";
    

    if(!empty($Apellido)){ 
    	$Persona= mysqli_real_escape_string($conexion,$Apellido);
    	$Consulta=  "SELECT * FROM pacientes_registrados INNER JOIN doctores_pacientesregistrados ON pacientes_registrados.ID_PR=doctores_pacientesregistrados.ID_PR WHERE apellido_pacient LIKE '$Persona%' AND ID_Doctor ='$Especialista' " ;
